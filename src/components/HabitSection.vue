...
const {
  ...
  skipHabitOnDate, removeHabitFromDate, addHabit,
...
// ── Add form ──────────────────────────────────────────────────────────────────
const HABIT_POOL = [
  'Morning meditation','Evening walk','Cold shower','No phone first hour',
  'Sleep by 11pm','Gratitude list','Read 10 pages','10 min stretch',
  'Cook at home','No social media','7+ hours sleep','Vitamins',
  'No sugar','10k steps','Journaling',
]
...
function confirmAdd() {
  ...
  addHabit(newText.value, newCadence.value, customDays.value, props.dateKey, newAreaId.value)
  closeForm()
}
function quickAdd(text) {
  addHabit(text, newCadence.value, customDays.value, props.dateKey, newAreaId.value)
  closeForm()
}
</script>
...
<template>
  <section aria-label="Habits">
    ...
    <div class="flex items-center justify-between mb-3">
      <h3 class="text-[13px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Habits</h3>
      <button
        v-if="!readOnly"
    ...
    <Transition name="slide-down">
      <div v-if="showForm" class="mb-3 flex flex-col gap-2">
        ...
        <div class="flex gap-1" role="group" aria-label="Habit cadence">
          <button
            v-for="c in CADENCES" :key="c.value"
            class="px-3 py-1 rounded-full text-xs border border-[var(--border)] text-[var(--text-muted)]
                   transition-colors hover:border-[var(--accent)] hover:text-[var(--text)]
                   focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
            :class="{ 'border-[var(--accent)] text-[var(--text)] bg-[var(--accent-soft)]': newCadence === c.value }"
            @click="newCadence = c.value"
            :aria-pressed="newCadence === c.value"
          >{{ c.label }}</button>
        </div>
        <!-- Custom day picker -->
        <div v-if="newCadence === 'custom'" class="flex gap-1" role="group" aria-label="Custom days">
          <button
            v-for="(d, i) in DAY_LABELS" :key="i"
            class="w-8 h-8 flex items-center justify-center rounded-full text-[11px] font-semibold
                   border border-[var(--border)] text-[var(--text-muted)] transition-colors
                   hover:border-[var(--accent)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
            :class="{ 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--text)]': customDays.includes(i) }"
        ...
        <!-- Area picker -->
        <div v-if="areas.length" class="flex flex-wrap gap-1" role="group" aria-label="Select area for habit">
          <button
    ...
    <TransitionGroup
      ...
      role="list" aria-label="Habits list"
    >
      <li
        v-for="habit in dateHabits" :key="habit.id"
        class="group flex items-center gap-2 px-2 py-2 rounded-[10px] transition-colors"
        :class="{
          'hover:bg-[var(--surface3)]':   !isHabitDone(dateKey, habit.id),
          'bg-[var(--surface2)] opacity-70': isHabitDone(dateKey, habit.id),
        }"
        role="listitem"
      ...
      >
        <button
          ...
          class="w-[18px] h-[18px] flex-shrink-0 flex items-center justify-center rounded-full
                 ...
                 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
          :class="{
            'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]':  isHabitDone(dateKey, habit.id),
            'border-[var(--border)] bg-[var(--surface2)] text-[var(--text)] hover:border-[var(--accent)]': !isHabitDone(dateKey, habit.id),
          }"
          @click="toggleHabit(habit.id)"
          :aria-label="(isHabitDone(dateKey, habit.id) ? 'Mark not done: ' : 'Mark done: ') + habit.text"
        ...
        <span
          v-else
          class="flex-shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded border"
          :class="{
            'border-[var(--accent)] text-[var(--accent)]':     isHabitDone(dateKey, habit.id),
            'border-[var(--border)] text-[var(--text-muted)]': !isHabitDone(dateKey, habit.id),
          }"
        ...
        <span
          ...
          :class="{
            'text-[var(--text-muted)] line-through': isHabitDone(dateKey, habit.id),
            'text-[var(--text)]':                    !isHabitDone(dateKey, habit.id),
          }"
        >{{ habit.text }}</span>

        <!-- Cadence badge -->
        <span class="text-[10px] font-medium text-[var(--text-sub)] flex-shrink-0 px-1.5 py-0.5 rounded border border-[var(--border)]">
          {{ cadenceLabel(habit) }}
        </span>

        <!-- Delete popover (active mode only) -->
        <div class="relative flex-shrink-0" v-if="!readOnly">
          <button
          ...
          <Transition name="fade">
            <div
              ...
              class="absolute right-0 top-full mt-1 z-20 min-w-[160px] rounded-xl
                     border border-[var(--border)] bg-[var(--surface)] p-1 flex flex-col gap-0.5"
            ...
            >
              ...
              <button
                class="w-full text-left px-3 py-2 rounded-lg text-[13px] text-[var(--danger)]
                       transition-colors hover:bg-[var(--danger-soft)]
                       focus:outline-none focus:ring-1 focus:ring-[var(--danger)]"
                @click="deleteFromHere(habit.id)"
                role="menuitem"
              >This day &amp; all future</button>
            </div>
          </Transition>
        </div>
    ...
    <p v-if="!dateHabits.length" class="text-[13px] text-[var(--text-sub)] px-2 py-1">
      No habits yet.
    </p>
