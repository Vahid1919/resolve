...
<template>
  <section
    ...
    class="bg-(--surface) border border-(--border) rounded-2xl p-7
           min-w-75 max-w-140 w-full shrink-0 animate-slide-up
           max-md:max-w-full"
  ...
  >
    ...
    >
      ...
      <div class="flex gap-1.5">
        ...
        >&#8250;</button>
        <button
          class="h-9 px-3.5 rounded-[9px] border border-(--border)
                 bg-(--surface2) text-(--text-muted) text-sm font-semibold
                 cursor-pointer transition-colors
                 hover:bg-(--surface3) hover:border-(--accent) hover:text-(--text)
                 focus:outline-none focus:ring-2 focus:ring-(--accent)"
          @click="jumpToToday"
    ...
    >
      ...
      <div
        ...
        class="text-center text-[11px] font-semibold text-(--text-muted)
      ...
      <div
        ...
        class="min-h-[62px] rounded-[10px] flex flex-col items-center justify-center
               cursor-pointer border-[1.5px] border-transparent bg-(--surface2)
               transition-all duration-200 select-none px-1 py-1.5 gap-0.5
               focus:outline-none focus:ring-2 focus:ring-(--accent)"
        :class="{
          'opacity-50':                 isPast(d),
          'border-(--accent)':     isSelected(d),
          'hover:border-(--accent) hover:scale-[1.06] hover:z-[2]': !isPast(d),
