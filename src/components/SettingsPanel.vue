...
<template>
  ...
  <section
    ...
    aria-modal="false"
    aria-label="Settings"
    class="absolute top-[calc(100%+8px)] right-0 w-64 rounded-xl border border-[var(--border)]
           bg-[var(--surface)] p-3 z-50 flex flex-col gap-3"
    style="box-shadow: 0 8px 28px rgba(0,0,0,0.45)"
  >

    <!-- ── Theme toggle ──────────────────────────────── -->
    <div class="flex items-center justify-between">
      <span class="text-[0.78rem] font-bold uppercase tracking-wider text-[var(--text-sub)]">
        Theme
      </span>
      <button
        class="flex items-center gap-1.5 text-[0.85rem] px-3 py-1 rounded-lg border
               border-[var(--border)] bg-[var(--surface2)] text-[var(--text)]
               transition-colors hover:bg-[var(--surface3)] hover:border-[var(--accent)]
    ...
    <!-- ── Areas ─────────────────────────────────────── -->
    <div>
      <h2 class="text-[0.78rem] font-bold uppercase tracking-wider text-[var(--text-sub)] mb-2">
        Areas
      </h2>

      <ul v-if="areas.length" class="flex flex-col gap-1 mb-3" role="list" aria-label="Saved areas">
        <li
          v-for="a in areas"
          :key="a.id"
        ...
        >
          <!-- Color-picker dot -->
          <label
            class="area-color-input w-2.5 h-2.5 rounded-full flex-shrink-0 relative
                   inline-block transition-transform cursor-pointer hover:scale-125"
          ...
          <template v-if="editingAreaId === a.id">
            <input
              class="area-edit-input flex-1 text-[0.9rem] px-2 py-0.5 rounded-md
                     border border-[var(--accent)] bg-[var(--surface2)] text-[var(--text)]
          ...
          <!-- Display state -->
          <template v-else>
            <span class="flex-1 text-[0.9rem] text-[var(--text)]">{{ a.name }}</span>

            <!-- Edit button -->
            <button
              class="p-0.5 rounded opacity-0 group-hover:opacity-100 focus:opacity-100
                     text-[var(--text-muted)] hover:text-[var(--accent)] hover:bg-[var(--accent-soft)]
                     transition-all focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
              @click="startEditArea(a)"
              :aria-label="`Rename area ${a.name}`"
            >
              <svg viewBox="0 0 14 14" fill="none" width="12" height="12" aria-hidden="true">
                <path d="M9.5 1.5l3 3-8 8H1.5v-3l8-8z" stroke="currentColor"
            ...
            <button
              class="p-0.5 rounded opacity-0 group-hover:opacity-100 focus:opacity-100
                     ...
                     transition-all focus:outline-none focus:ring-1 focus:ring-[var(--danger)]"
      ...
      <p v-else class="text-[0.85rem] text-[var(--text-muted)] mb-3">No areas yet.</p>

      <!-- Add area form ──────────────────────────────── -->
      <div class="flex flex-col gap-2">

        <!-- Color swatches -->
        <div class="flex gap-1 flex-wrap" role="group" aria-label="Choose area colour">
          <button
            v-for="c in PRESET_COLORS"
            :key="c"
            class="w-4 h-4 rounded-full border-2 border-transparent transition-transform
                   hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-1
                   focus:ring-[var(--accent)]"
            :class="{ 'border-[var(--text)] scale-110': newAreaColor === c }"
            :style="{ background: c }"
            :aria-label="`Colour ${c}`"
          ...
          <!-- Custom color wheel -->
          <label
            class="color-wheel-swatch w-4 h-4 rounded-full border-2 border-transparent
                   transition-transform hover:scale-110"
            :class="{ 'border-[var(--text)] scale-110': !PRESET_COLORS.includes(newAreaColor) }"
            :style="!PRESET_COLORS.includes(newAreaColor)
        ...
        <div class="flex gap-1.5">
          <input
            class="flex-1 px-3 py-2 rounded-lg border border-[var(--border)]
                   bg-[var(--surface2)] text-[var(--text)] text-sm
                   placeholder:text-[var(--text-sub)] outline-none
                   focus:border-[var(--accent)] focus:bg-[var(--surface3)]
          ...
          <button
            class="px-4 py-2 rounded-lg bg-[var(--accent)] text-[var(--bg)] text-sm font-semibold
                   transition-all hover:bg-[var(--accent-dim)] hover:scale-[1.04] active:scale-[0.97]
